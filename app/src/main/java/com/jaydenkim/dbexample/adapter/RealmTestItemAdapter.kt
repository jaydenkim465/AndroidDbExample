...
class RealmTestItemAdapter(private var dataSet: List<RealmTestItem>) :
	RecyclerView.Adapter<RealmTestItemAdapter.TextItemViewHolder>() {
	class TextItemViewHolder(private val binding: TestItemLayoutBinding) :
		RecyclerView.ViewHolder(binding.root) {
		fun bind(item: RealmTestItem) {
			binding.dbIndex.text = ""//item._id.timestamp.toString()
